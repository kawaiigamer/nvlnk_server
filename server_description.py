...
class Param(OrderedDataclass):
    name: str
    fullname: str
    default_value: Union[str, int, np.dtype, bytes]
    description: str
    data_type: Type = None
...
_endpoints = {"wav": RoutePart("Uncompressed audio",
                               {
                                "random": RoutePart("Streams using random data as source", {
                                                                            ...
                                                                            "N-FSK": RoutePart("N-FSK(Frequency Shift Keying) modulation", {
                                                                                                                 "stream": EndpointPart("N-FSK audio/wav stream", "GET", _wav_params_descr + _wav_duration_params_descr + _wav_additional_params_descr + _wav_nfsk_params_descr + _wav_nfsk_level_params_descr, presets=_fsk_presets),
                                                                                                                 }),
                                                                            "aes256_N-FSK": RoutePart("AES-256(GCM/CBC)+N-FSK(Frequency Shift Keying) modulation", {
                                                                                                                 "stream": EndpointPart("AES-256(GCM/CBC)+N-FSK audio/wav stream", "GET", _wav_params_descr + _wav_duration_params_descr + _wav_additional_params_descr + _wav_nfsk_params_descr + _wav_nfsk_level_params_descr + _aes_params_descr),
                                                                                                                 }),
                                                                            },),
...
def get_wav_fsk_params(args) -> Optional[Dict[str, Any]]:
    if preset := _fsk_presets.get(args.get("preset")):
        return {**preset, **__get_params_from_request(args, _wav_duration_params_descr + _wav_nfsk_level_params_descr + _wav_additional_params_descr + _wav_nfsk_decrypt_errors_descr)}
    return __get_params_from_request(args, _wav_params_descr + _wav_duration_params_descr + _wav_nfsk_params_descr + _wav_nfsk_level_params_descr + _wav_additional_params_descr + _wav_nfsk_decrypt_errors_descr)
